        ...
        <div class="col-md-12">
            @can('habit_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.habits.create') }}">
                            ...
                            <tbody>
                                @foreach($habits as $key => $habit)
                                    <tr data-entry-id="{{ $habit->id }}">
                                        ...
                                        <td>
                                            @can('habit_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.habits.show', $habit->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('habit_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.habits.edit', $habit->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('habit_delete')
                                                <form action="{{ route('frontend.habits.destroy', $habit->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('habit_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.habits.massDestroy') }}",
