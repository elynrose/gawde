        ...
        <div class="col-md-12">
            @can('user_habit_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-habits.create') }}">
                            ...
                            <tbody>
                                @foreach($userHabits as $key => $userHabit)
                                    <tr data-entry-id="{{ $userHabit->id }}">
                                        ...
                                        <td>
                                            @can('user_habit_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-habits.show', $userHabit->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_habit_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-habits.edit', $userHabit->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_habit_delete')
                                                <form action="{{ route('frontend.user-habits.destroy', $userHabit->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_habit_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-habits.massDestroy') }}",
