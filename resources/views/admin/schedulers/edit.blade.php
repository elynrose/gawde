...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.scheduler.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.schedulers.update", [$scheduler->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('habit') ? 'is-invalid' : '' }}" name="habit_id" id="habit_id" required>
                    @foreach($habits as $id => $entry)
                        <option value="{{ $id }}" {{ (old('habit_id') ? old('habit_id') : $scheduler->habit->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('habit'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('today') ? 'is-invalid' : '' }}" type="text" name="today" id="today" value="{{ old('today', $scheduler->today) }}" required>
                @if($errors->has('today'))
                    <div class="invalid-feedback">
                        {{ $errors->first('today') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $scheduler->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('reminded') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="reminded" id="reminded" value="1" {{ $scheduler->reminded || old('reminded', 0) === 1 ? 'checked' : '' }}>
