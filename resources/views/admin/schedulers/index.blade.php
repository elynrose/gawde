...
@can('scheduler_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.schedulers.create') }}">
                ...
                <tbody>
                    @foreach($schedulers as $key => $scheduler)
                        <tr data-entry-id="{{ $scheduler->id }}">
                            ...
                            <td>
                                @can('scheduler_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.schedulers.show', $scheduler->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('scheduler_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.schedulers.edit', $scheduler->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('scheduler_delete')
                                    <form action="{{ route('admin.schedulers.destroy', $scheduler->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('scheduler_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.schedulers.massDestroy') }}",
